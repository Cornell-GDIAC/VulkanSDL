/**
 * Vulkan Tutorial: Part 4
 *
 * This code is the texture mapping tutorial from https://vulkan-tutorial.com.
 * It is the material up to and including the section on the combined image.
 * sampler. See README.md for the changes made to adapt this tutorial to SDL.
 */
...
private:
    ...
    void createTextureImage() {
        ...
        uint8_t* pixels = load_image_asset(TEXTURE_PATH.c_str(), &texWidth, &texHeight);
    ...
    void loadModel() {
        ...
        std::string path = get_asset(MODEL_PATH);
...
#define BUFF_SIZE 1024
        ...
        if (!tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err, &istream)) {
            SDL_Log("%s %s",warn.c_str(),err.c_str());
        ...
        for (const char* layerName : validationLayers) {
            ...
            for (const auto& layerProperties : availableLayers) {
                if (strcmp(layerName, layerProperties.layerName) == 0) {
                    print_version(layerProperties.layerName,
...
public:
    ...
    ModelApplication() : window(NULL) {}
    
    ~ModelApplication() { cleanup(); }
    
    bool setup() {
        // Set the basic metadata
        if (!SDL_SetAppMetadata("Vulkan Tutorial", "1.0.0","com.vulkan-tutorial.tutorial6")) {
...
SDL_AppResult SDL_AppIterate(void *appstate) {
    ModelApplication* app = (ModelApplication*)appstate;
    try {
        app->run();
    } catch (const std::exception& e) {
        std::cerr << e.what() << std::endl;
